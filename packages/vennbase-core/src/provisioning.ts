...
export class Provisioning {
  ...
  private async ensureFederationWorkerUrl(
    ...
    if (hadPriorWorkerMetadata) {
      ...
      console.info(
        `[vennbase] deploying federation worker ${workerName} for ${username} on ${appHostname} at runtime ${CLASSIC_WORKER_RUNTIME_ID}`,
      );
    }
    ...
    console.info(
      `[vennbase] federation worker ready ${workerName} runtime ${CLASSIC_WORKER_RUNTIME_ID} ${activeWorkerUrl}`,
    );
  ...
  private async deployWorker(args: DeployWorkerArgs): Promise<string | undefined> {
    ...
    try {
      ...
      console.error("[vennbase] deployWorker failed", {
        error,
        workerName,
        workerFilePath,
      });
