...
function workerMetadataKey(kind: "runtime" | "url", hostHash: string): string {
  return `vennbase:federation-worker-${kind}:v2:owner:${hostHash}`;
}
...
describe("Vennbase", () => {
  ...
  it("returns signed-out session without triggering interactive login", async () => {
    ...
    await expect(db.whoAmI()).rejects.toMatchObject<Partial<VennbaseError>>({
      code: "SIGNED_OUT",
    });
  ...
  it("ignores legacy username-prefixed remembered rows", async () => {
    const kv = new MapKv();
    await kv.set(
      "vennbase:saved-row:v1:owner:current-row",
      JSON.stringify({
        id: "owner_row",
        collection: "rows",
        baseUrl: "https://worker.example",
  ...
  it("explains how to provide Puter when signIn is called without a backend", async () => {
    ...
    await expect(db.signIn()).rejects.toMatchObject<Partial<VennbaseError>>({
      code: "SIGNED_OUT",
  ...
  it("throws VennbaseError for API failures", async () => {
    const db = new Vennbase({
      schema: MINIMAL_SCHEMA,
      identityProvider: async () => ({ username: "owner" }),
  ...
  it("starts provisioning a host-scoped federation worker from constructor prewarm", async () => {
    ...
    expect(consoleInfo).toHaveBeenCalledWith(
      `[vennbase] deploying federation worker ${expectedWorkerName} for owner on ${appHost} at runtime ${CLASSIC_WORKER_RUNTIME_ID}`,
    );
    expect(consoleInfo).toHaveBeenCalledWith(
      `[vennbase] federation worker ready ${expectedWorkerName} runtime ${CLASSIC_WORKER_RUNTIME_ID} ${deployedWorkerBase}`,
    );
