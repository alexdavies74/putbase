...
export async function signCanonicalValue(value: unknown, privateKey: CryptoKey): Promise<string> {
  const bytes = toArrayBuffer(canonicalBytes(value));
...
export async function verifyCanonicalValue(
  ...
  const signature = toArrayBuffer(fromBase64Url(signatureBase64Url));
...
export function signaturePayload<TPayload extends object>(
  action: SignedAction,
  payload: TPayload,
...
export async function signEnvelope<TPayload extends object>(
  ...
  const payloadToSign = signaturePayload(action, payload, signer, signedAt);
  const signature = await signCanonicalValue(payloadToSign, privateKey);
...
export async function verifyEnvelope<TPayload extends object>(
  ...
  const payloadToVerify = signaturePayload(
    envelope.action,
    envelope.payload,
    envelope.signer,
    envelope.signedAt,
  ...
  return verifyCanonicalValue(payloadToVerify, envelope.signature, publicKey);
}

export function buildPublicKeyProofDocument(
  username: string,
  publicKeyJwk: JsonWebKey,
): PublicKeyProofDocument {
  return {
...
export function encodeProofDocumentAsDataUrl(document: PublicKeyProofDocument): string {
...
export function decodeDataUrlJson<T>(dataUrl: string): T {
